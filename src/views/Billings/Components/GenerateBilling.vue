    ...
    <div class="mt-3">
      ...
      >
        ...
        <option
          ...
          :value="program.id"
        >
          Program: {{ program.grant }} || Batch No: {{ program.batch_no }} ||
          Batch type: {{ program.batch_type }}
        </option>
      </select>

      <p class="text-gray-900 text-right font-bold text-sm mt-3">
        No. of Grantees: {{ formatDecimalValue(selectedGrantees.length) }}
      </p>
    </div>

    <table
      v-if="grantees && grantees.data && grantees.data.length > 0"
      ...
      <div class="grid grid-cols-6">
        ...
        <div class="col-end-8 gap-3">
          ...
          <button
            ...
            class="bg-orange-600 hover:bg-orange-700 text-white text-sm py-2 px-4 rounded-lg mr-3"
          >
            Export Grantees
          </button>
          <button
  ...
  methods: {
        ...
        this.$api
          ...
          .then((res) => {
            if (res.data.success) {
              ...
              this.$toast.error(
                res.data.message || "Failed to generate billing."
              );
            }
          })
          .catch((error) => {
    ...
    updateBillingForm() {
      const selectedProgram = this.programs.find(
        (program) => program.id === this.programId
      );
