<template>
    <!-- Timeline Section -->
    <section class="bg-gray-50 rounded-lg p-4 overflow-y-auto border border-gray-200" style="height: 460px;">
      <ul class="space-y-4">
        <li
        ...
        >
          <h6 class="text-base font-semibold text-gray-800">{{ remark.remarks }}</h6>
          <p class="text-xs text-gray-500 font-medium">
            {{ formatDate(remark.created_at) }} &nbsp;|&nbsp; by: {{ remark.user_name }}
          </p>
...
export default {
    ...
    props: {
        voucherId: {
            type: String, // Specify the expected type(s)
...
async fetchRemarks() {
  try {
    const [voucherRes, billingRes] = await Promise.all([
      this.$api.get(`/remarks`, {
    ...
    // Combine and sort oldest → newest
    const merged = [
      ...voucherRes.data.map(r => ({ ...r, source: 'voucher' })),
      ...billingRes.data.map(r => ({ ...r, source: 'billing' }))
    ].sort((a, b) => new Date(a.created_at) - new Date(b.created_at))

    this.remarks = merged
    this.$emit('remarks_load_status', this.remarks_load)
  } catch (error) {
    console.error('Failed to fetch merged remarks:', error)
  }
},
